...
- **`orm.server.ts`**: ORM/Drizzle-схемы домена (только сервер, с директивой 'server-only')
- **`types.shared.ts`**: Типы и Zod-схемы (универсальный, без серверных зависимостей)
- **`model/enums.ts`**: Доменные перечисления и константы
...
Это обеспечивает чистое разделение серверной логики и универсальных типов/валидации, а также централизованное управление схемами через Drizzle.
